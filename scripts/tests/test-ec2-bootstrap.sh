...
assert_file_contains 'echo "::add-mask::$EC2_PUBLIC_IP"' "$WORKFLOW"
assert_file_contains 'echo "public_ip=$EC2_PUBLIC_IP" >> "$GITHUB_OUTPUT"' "$WORKFLOW"
assert_file_contains 'EC2_PUBLIC_IP: ${{ steps.ec2_address.outputs.public_ip }}' "$WORKFLOW"
assert_file_contains ': "${EC2_PUBLIC_IP:?Resolved EC2 public IPv4 address is unavailable}"' "$WORKFLOW"
assert_file_not_contains 'EC2_HOST=' "$WORKFLOW"
...
assert_file_contains 'PUBLIC_IP="$4"' "$SSM_DEPLOYMENT_WRAPPER"
assert_file_contains 'APP_URL="$app_url" EC2_APP_DIR="$app_dir"' "$COORDINATOR"
assert_file_not_contains 'EC2_HOST=' "$COORDINATOR"
